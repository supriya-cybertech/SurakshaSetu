...
from database import SessionLocal, get_db, IncidentLog, Resident, Visitor, AccessLog, CameraConfig
from AI.tailgating_logic import TailgatingDetector, TailgatingAlert
...
async def handle_tailgating_alert(alert: TailgatingAlert, db=None):
    ...
    if db:
        try:
            ...
            db.add(incident)
            db.commit()
            logger.info(f"Incident logged to database: {incident.id}")
        except Exception as e:
    ...
    system_state.broadcast_alert(alert_payload)
    ...
    try:
        while True:
            ...
            try:
                ...
                if alert:
                    asyncio.run(handle_tailgating_alert(alert))
                ...
                if detection_results["weapons"]:
                    ...
                    system_state.broadcast_alert(weapon_data)
            ...
            if frame_count % 5 == 0:
                try:
                    ...
                    system_state.broadcast_alert(frame_data)
                except Exception as e:
...
async def startup_event():
    ...
    # Start mock camera streams
    cameras = [
        (1, "rtsp://example.com/camera1"),  # IP Camera 1
        (2, "rtsp://example.com/camera2"),  # IP Camera 2
        (3, "rtsp://example.com/camera3"),  # IP Camera 3 (Entry Point)
        (4, "rtsp://example.com/camera4"),  # IP Camera 4
        (0, 0),  # Webcam
    ]
    
    # For demo, use test video or mock streams
    # In production, use actual RTSP URLs from IP cameras
    
    for camera_id, stream_url in cameras:
        if stream_url == 0:
...
async def get_dashboard_stats(db = Depends(get_db)):
    ...
    try:
        ...
        high_severity = db.query(IncidentLog).filter(IncidentLog.severity == SeverityLevel.HIGH).count()
...
async def get_incidents(
    ...
    """Get incident log"""
    try:
        query = db.query(IncidentLog).order_by(IncidentLog.timestamp.desc())
            ...
            "incidents": [
                {
                    ...
                    "details": inc.additional_details,
                    "resolved": inc.resolved
                }
                for inc in incidents
            ],
            "total": db.query(IncidentLog).count()
        }
    except Exception as e:
